...
export default async function EmployeesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const wageEdit = can(auth, "wage.edit");
  return <OperationsFrame auth={auth} active="/employees"><div className="page operations-page">
    <section className="welcome page-title"><div><p className="eyebrow">PEOPLE & ACCESS</p><h1>Employees</h1><p>Canonical employee records, login links, roles, crews, and protected wage data.</p></div>{manage && <Link className="primary button-anchor" href="/employees?new=1#employee-form">＋ Add employee</Link>}</section>
    ...
    <div className="operations-grid">
      ...
      {manage && <section className="panel editor-panel" id="employee-form"><span className="section-mark">{selected ? "EDIT EMPLOYEE" : "NEW EMPLOYEE"}</span><h2>{selected ? `${selected.first_name} ${selected.last_name}` : "Employee details"}</h2><form action="/api/operations" method="post" className="operations-form"><input type="hidden" name="action" value="employee_save"/><input type="hidden" name="id" value={selected?.id ?? ""}/><input type="hidden" name="returnTo" value={selected ? `/employees?edit=${selected.id}` : "/employees"}/><div className="form-pair"><label>Employee ID<input name="employeeNumber" defaultValue={selected?.employee_number} required/></label><label>Role<select name="roleId" defaultValue={selected?.role_id} required>{roles.map(role => <option value={role.id} key={role.id}>{role.name}</option>)}</select></label></div><div className="form-pair"><label>First name<input name="firstName" defaultValue={selected?.first_name} required/></label><label>Last name<input name="lastName" defaultValue={selected?.last_name} required/></label></div><div className="form-pair"><label>Phone<input name="phone" type="tel" defaultValue={selected?.phone ?? ""}/></label><label>Email<input name="email" type="email" defaultValue={selected?.email ?? ""}/></label></div>{wageEdit && <><div className="form-pair wage-fields"><label>Hourly wage<input name="wage" inputMode="decimal" defaultValue={selected?.default_hourly_wage_cents != null ? (selected.default_hourly_wage_cents / 100).toFixed(2) : ""} placeholder="0.00"/></label><label>Effective date<input name="wageEffectiveDate" type="date" defaultValue={selected?.wage_effective_date ?? new Date().toISOString().slice(0,10)}/></label></div>{selected && <label>Wage-change reason<input name="wageReason" placeholder="Required for a meaningful audit trail"/></label>}</>}<label>Employment notes<textarea name="notes" defaultValue={selected?.notes ?? ""} rows={4}/></label><button className="primary" type="submit">Save employee</button></form>{selected && <form action="/api/operations" method="post" className="state-form"><input type="hidden" name="action" value="state"/><input type="hidden" name="entity" value="employee"/><input type="hidden" name="id" value={selected.id}/><input type="hidden" name="active" value={String(!selected.active)}/><input type="hidden" name="returnTo" value={`/employees?edit=${selected.id}`}/><button type="submit" className="text-button danger-link">{selected.active ? "Deactivate employee" : "Reactivate employee"}</button></form>}</section>}
    </div>{wageView && !wageEdit && <p className="security-note">Wage data is visible to you but requires wage-edit permission to change.</p>}
  </div></OperationsFrame>;
}
