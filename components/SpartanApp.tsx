...
type RecordLinks = { projects: Record<string, string>; employees: Record<string, string>; crews: Record<string, string> };
...
export function SpartanApp({ userName, roleName, organizationName, canAdmin, isPlatformAdmin, recordLinks }: { userName: string; roleName: string; organizationName: string; canAdmin: boolean; isPlatformAdmin: boolean; recordLinks: RecordLinks }) {
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main">
        ...
        {view === "dashboard" && <Dashboard clock={clock} nextClockAction={nextClockAction} setClock={setClock} notify={notify} recordLinks={recordLinks} />}
        {view === "projects" && <Projects search={search} setSearch={setSearch} items={filteredProjects} notify={notify} recordLinks={recordLinks} />}
        ...
        {view === "team" && <Team notify={notify} recordLinks={recordLinks} />}
...
function Dashboard({ clock, nextClockAction, setClock, notify, recordLinks }: { clock: ClockState; nextClockAction: () => void; setClock: (s: ClockState) => void; notify: (s: string) => void; recordLinks: RecordLinks }) {
  return <div className="page dashboard-page">
    ...
    <section className="grid two-one">
      <div className="panel today-panel">
        ...
        {[{n:"Justin Interior Crew",p:"Smith Residence",c:"6 workers",t:"7:00 AM – 3:30 PM",i:"JI",tone:"navy",kind:"crew"},{n:"Mercer Finish Crew",p:"Mercer Offices",c:"3 workers",t:"6:30 AM – 3:00 PM",i:"MF",tone:"rust",kind:"crew"},{n:"Jake Morrison",p:"Johnson Remodel",c:"Individual assignment",t:"1:00 PM – 4:00 PM",i:"JM",tone:"olive",kind:"employee"}].map((row, idx) => <div className="crew-row" key={row.n}><span className={`crew-badge ${row.tone}`}>{row.i}</span><div className="crew-main"><strong>{row.n}</strong><span>{row.c}</span></div><div className="crew-job"><strong>{row.p}</strong><span>{idx === 0 ? "3427 Hawthorne Ave" : idx === 1 ? "620 SW Fifth Ave" : "118 SE Alder St"}</span></div><time>{row.t}</time><a className="row-open-link" aria-label={`Open ${row.n}`} href={row.kind === "crew" ? recordLinks.crews[row.n] ?? "/crews" : recordLinks.employees[row.n] ?? "/employees"}>›</a></div>)}
      </div>
...
function Projects({ search, setSearch, items, recordLinks }: { search:string;setSearch:(s:string)=>void;items:typeof projects;notify:(s:string)=>void;recordLinks:RecordLinks }) { return <div className="page"><section className="welcome page-title"><div><p className="eyebrow">PORTFOLIO</p><h1>Projects</h1><p>Manage jobsites, teams, labor, and closeout work.</p></div><a className="primary button-anchor" href="/projects/new">＋ New project</a></section><div className="toolbar"><label className="search">⌕<input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search projects…" /></label><button className="filter">Active projects⌄</button></div><div className="project-cards">{items.map(p=><a className="project-card" key={p.code} href={recordLinks.projects[p.name] ?? "/projects"}><div className="project-card-top"><span className={`status-pill ${p.stageClass}`}>{p.stage}</span><span>{p.code}</span></div><h2>{p.name}</h2><p>{p.meta}</p><div className="project-card-stats"><span><small>WORKERS</small><strong>{p.people}</strong></span><span><small>LABOR HRS</small><strong>{p.hours.toLocaleString()}</strong></span><span><small>OPEN PUNCH</small><strong>{p.punch}</strong></span></div><div className="project-progress"><span><i style={{width:`${p.progress}%`}} /></span><b>{p.progress}%</b></div></a>)}</div></div>; }
...
function Team({recordLinks}:{notify:(s:string)=>void;recordLinks:RecordLinks}) { return <div className="page"><section className="welcome page-title"><div><p className="eyebrow">PEOPLE & CREWS</p><h1>Team</h1><p>Manage employees, permissions, crews, and wage history.</p></div><a className="primary button-anchor" href="/employees?new=1#employee-form">＋ Add employee</a></section><div className="team-grid">{[{n:"Jake Morrison",r:"Foreman",c:"Justin Interior Crew",s:"On job",i:"JM"},{n:"Carlos Ruiz",r:"Employee",c:"Justin Interior Crew",s:"On job",i:"CR"},{n:"Mike Taylor",r:"Employee",c:"Mercer Finish Crew",s:"On job",i:"MT"},{n:"Sam Kim",r:"Employee",c:"Justin Interior Crew",s:"On job",i:"SK"},{n:"Dana Brooks",r:"Manager",c:"Operations",s:"Active",i:"DB"},{n:"Olivia Chen",r:"Viewer",c:"Office",s:"Active",i:"OC"}].map(e=><a className="team-card" key={e.n} href={recordLinks.employees[e.n] ?? "/employees"}><span className="avatar large slate">{e.i}</span><div><h3>{e.n}</h3><p>{e.r}</p></div><span className="status-pill active">{e.s}</span><small>{e.c}</small></a>)}</div></div>; }
