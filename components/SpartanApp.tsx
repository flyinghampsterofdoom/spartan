...
const punchItems = [
  { id: "P-184", place: "Primary bathroom", title: "Touch up paint above vanity", person: "Carlos Ruiz", due: "Today", status: "Needs review", cls: "review" },
  { id: "P-191", place: "Kitchen", title: "Adjust north cabinet door", person: "Jake Morrison", due: "Today", status: "In progress", cls: "progress" },
  { id: "P-176", place: "Living room", title: "Repair trim at west window", person: "Justin Interior", due: "Jul 13", status: "Rework", cls: "rework" },
];
    ...
    <div className="app-shell">
      <aside className={mobileNav ? "sidebar open" : "sidebar"}>
        ...
        <nav aria-label="Primary navigation">
          ...
          {nav.map(item => item.id === "projects" || item.id === "schedule" || item.id === "time" || item.id === "team"
            ? <a key={item.id} className="nav-item" href={item.id === "projects" ? "/projects" : item.id === "schedule" ? "/schedule" : item.id === "time" ? "/time" : "/employees"}><span>{item.glyph}</span>{item.label}</a>
      ...
      <main className="main">
        ...
        {view === "punch" && <Punch notify={notify} />}
...
function Punch({notify}:{notify:(s:string)=>void}) { const [items,setItems]=useState(punchItems); function advance(id:string){setItems(items.map(i=>i.id===id?{...i,status:i.status==="In progress"?"Needs review":"Approved",cls:i.status==="In progress"?"review":"approved"}:i));notify("Punch item status updated");} return <div className="page"><PageTitle eyebrow="QUALITY & CLOSEOUT" title="Punch lists" detail="Track deficiencies from walk-through to verified completion." action="Start punch walk" onAction={()=>notify("Punch walk mode started · Primary bathroom")} /><div className="punch-summary"><Metric label="Open items" value="43" detail="across 3 projects" icon="✓" /><Metric label="Needs review" value="14" detail="ready for approval" icon="◉" tone="orange" /><Metric label="Overdue" value="5" detail="past due date" icon="!" tone="red" /></div><div className="panel punch-list-panel"><div className="panel-head"><div><span className="section-mark">SMITH RESIDENCE</span><h2>Final construction punch</h2></div><button className="filter">All statuses⌄</button></div>{items.map(item=><div className="punch-row" key={item.id}><span className="punch-photo">▧</span><div><small>{item.id} · {item.place}</small><strong>{item.title}</strong><span>{item.person} · Due {item.due}</span></div><span className={`status-pill ${item.cls}`}>{item.status}</span><button className="secondary compact" onClick={()=>advance(item.id)}>{item.status==="In progress"?"Complete":"Review"}</button></div>)}</div></div>; }
