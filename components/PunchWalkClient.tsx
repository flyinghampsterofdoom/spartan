...
export function PunchWalkClient({ projects, initialLists, areas, categories, assignees, initialProjectId = "" }: { projects: Project[]; initialLists: List[]; areas: Area[]; categories: Option[]; assignees: Assignee[]; initialProjectId?: string }) {
  ...
  async function saveItem() {
    ...
    try {
      ...
      if (file) {
        try { await uploadPhoto(saved.itemId, saved.eventId, file); finishCapture("Item and photo saved. Ready for the next item."); }
      ...
      } else finishCapture(body.duplicate ? "Item was already saved. Ready for the next item." : "Item saved. Ready for the next item.");
    } catch (error) { setMessage(`${error instanceof Error ? error.message : "Save failed."} Your item details are still here. Try again.`); } finally { setBusy(false); setProgress(null); }
  }
  ...
  function uploadPhoto(itemId: string, eventId: string | null, selected: File) {
    return prepareFieldPhoto(selected).then(file => uploadPunchPhoto(file, { ownerId: itemId, context: "initial_issue", relatedEventId: eventId }, value => setProgress(value))).then(() => undefined);
  }

  return <div className="punch-walk-shell"><header className="walk-header"><div><span className="section-mark">PUNCH WALK</span><h1>{project?.name ?? "Select a project"}</h1><p>Add items quickly as you walk the job.</p></div><div className="walk-count"><strong>{count}</strong><span>items</span></div></header><section className="panel walk-defaults"><span className="section-mark">WALK SETUP</span><div className="walk-default-grid"><label>Project<select value={defaults.projectId} onChange={event => selectProject(event.target.value)}><option value="">Select project</option>{projects.map(value => <option value={value.id} key={value.id}>{value.projectNumber} · {value.name}</option>)}</select></label><label>Punch list<select value={defaults.listId} onChange={event => update("listId", event.target.value)} disabled={!defaults.projectId}><option value="">Select list</option>{projectLists.map(value => <option value={value.id} key={value.id}>{value.name}</option>)}</select></label><label>Area<select value={defaults.areaId} onChange={event => update("areaId", event.target.value)} disabled={!defaults.projectId}><option value="">Unspecified</option>{projectAreas.map(value => <option value={value.id} key={value.id}>{value.name}</option>)}</select></label><label>Category<select value={defaults.categoryId} onChange={event => update("categoryId", event.target.value)}><option value="">General</option>{categories.map(value => <option value={value.id} key={value.id}>{value.name}</option>)}</select></label><label>Assignee<select value={defaults.assignee} onChange={event => update("assignee", event.target.value)}><option value="">Unassigned</option>{assignees.map(value => <option value={`${value.kind}:${value.id}`} key={`${value.kind}:${value.id}`}>{value.name}</option>)}</select></label><label>Priority<select value={defaults.priority} onChange={event => update("priority", event.target.value)}><option value="low">Low</option><option value="normal">Normal</option><option value="high">High</option><option value="urgent">Urgent</option></select></label></div>{defaults.projectId && <details className="walk-new-list"><summary>Create a new punch list</summary><div><input value={newListName} onChange={event => setNewListName(event.target.value)} placeholder="Punch list name"/><button type="button" className="secondary" disabled={busy || !newListName.trim()} onClick={createList}>Create list</button></div></details>}</section><section className="panel walk-capture"><span className="section-mark">NEW ITEM</span><label className="walk-description">What needs attention?<textarea value={description} onChange={event => setDescription(event.target.value)} rows={3} enterKeyHint="done" placeholder="Describe the issue. You can use your keyboard microphone." disabled={busy || Boolean(pendingPhoto)}/></label><div className="walk-photo-zone">{preview ? <img src={preview} alt="Selected punch item"/> : <button type="button" className="walk-camera-placeholder" onClick={() => cameraRef.current?.click()} disabled={busy || Boolean(pendingPhoto)}><b>＋ Add photo</b><span>Take a photo or choose one</span></button>}<div><button type="button" className="secondary" onClick={() => cameraRef.current?.click()} disabled={busy || Boolean(pendingPhoto)}>Take photo</button><button type="button" className="text-button" onClick={() => libraryRef.current?.click()} disabled={busy || Boolean(pendingPhoto)}>Choose photo</button>{file && !pendingPhoto && <button type="button" className="text-button danger-link" onClick={() => selectPhoto(null)}>Remove photo</button>}</div><input ref={cameraRef} className="visually-hidden" type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" capture="environment" onChange={event => selectPhoto(event.target.files?.[0] ?? null)}/><input ref={libraryRef} className="visually-hidden" type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" onChange={event => selectPhoto(event.target.files?.[0] ?? null)}/></div>{progress != null && <div className="upload-progress"><span style={{ width: `${progress}%` }}/><b>{progress}%</b></div>}{message && <p className={`walk-message ${message.includes("failed") || message.includes("rejected") ? "error" : ""}`} role="status">{message}</p>}{pendingPhoto ? <div className="partial-failure-actions"><button className="primary" type="button" disabled={busy} onClick={retryPhoto}>Retry photo</button><button className="secondary" type="button" disabled={busy} onClick={continueWithoutPhoto}>Continue without photo</button></div> : <button className="primary walk-save" type="button" disabled={busy || !defaults.projectId || !defaults.listId || !description.trim()} onClick={saveItem}>{busy ? "Saving…" : "Save + add another"}</button>}</section></div>;
}
