...
const links = [
  ["/", "⌂", "Overview"], ["/projects", "▱", "Projects"], ["/crews", "◫", "Crews"], ["/employees", "◎", "Employees"],
] as const;
...
export function OperationsFrame({ auth, active, children }: { auth: AuthContext; active: string; children: ReactNode }) {
  ...
  return <div className="app-shell operations-shell">
    <aside className="sidebar operations-sidebar">
      ...
      <nav aria-label="Primary navigation"><p className="nav-kicker">Workspace</p>{links.map(([href, glyph, label]) => <Link key={href} href={href} className={`nav-item ${active === href ? "selected" : ""}`}><span>{glyph}</span>{label}</Link>)}</nav>
      <div className="sidebar-foot"><div className="profile-mini"><span className="avatar amber">{initials}</span><div><strong>{auth.displayName}</strong><small>{auth.roleName} · {auth.organizationName}</small></div></div><div className="account-links"><Link href="/account">Account & sessions</Link><Link href="/settings">Company settings</Link><form action="/api/auth/logout" method="post"><button type="submit">Log out</button></form></div></div>
    </aside>
