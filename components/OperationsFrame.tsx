...
const links = [
  ["/", "⌂", "Overview"], ["/projects", "▱", "Projects"], ["/schedule", "□", "Schedule"], ["/time", "◷", "Time"], ["/punch", "✓", "Punch"], ["/crews", "◫", "Crews"], ["/employees", "◎", "Employees"], ["/wages", "¤", "Wages"],
] as const;
...
export function OperationsFrame({ auth, active, children }: { auth: AuthContext; active: string; children: ReactNode }) {
  ...
  const visibleLinks = links.filter(([href]) => href === "/"
    ...
    || (href === "/punch" && ["punch.view", "punch.work", "punch.manage", "punch.approve"].some(permission => auth.permissions[permission]?.allowed))
